...
7. Read `$REPO/agents/scientist/scientist-guidance.md` if it exists. The supervisor may have left strategic direction.
8. Initialise `agents/scientist/scientist-results.md` with just the header row if it does not exist yet, then commit it.
...
1. Read $REPO/agents/scientist/scientist-guidance.md
...
6. Decide: keep or discard
   ...
   - otherwise judge the run against the current lane in $REPO/agents/scientist/scientist-guidance.md
...
7. Append result to agents/scientist/scientist-results.md, then commit it.

8. Loop
```

**On crashes:** If the run errors due to something trivial (typo, missing import), fix `agents/scientist/experiment.py` and amend the commit before re-running. If the idea is fundamentally broken, discard, log as `error`, and move on.
...
`agents/scientist/scientist-results.md` is append-only and committed after every experiment. Add one row per run:
